...
class StillWatchingXML(BaseDialog):
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('zoro.tvshowtitle', self.meta.get('tvshowtitle'))
			self.setProperty('zoro.title', self.meta.get('title'))
			...
			self.setProperty('zoro.episode', str(self.meta.get('episode', '')))
			self.setProperty('zoro.rating', str(self.meta.get('rating', '')))
			self.setProperty('zoro.landscape', self.meta.get('landscape', ''))
			self.setProperty('zoro.fanart', self.meta.get('fanart', ''))
			self.setProperty('zoro.thumb', self.meta.get('thumb', ''))
			next_duration = int(self.meta.get('duration')) if self.meta.get('duration') else ''
			self.setProperty('zoro.duration', str(int(next_duration)))
			endtime = (datetime.now() + timedelta(seconds=next_duration)).strftime('%I:%M %p').lstrip('0') if next_duration else ''
			self.setProperty('zoro.endtime', endtime)
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
