...
def unHideItems(tvdb_ids):
	...
	try:
		...
		for section in sections:
			...
			control.sleep(1000)
		if success:
			if 'plugin.video.zoro' in control.infoLabel('Container.PluginName'): control.refresh()
	...
	try:
		...
		if select >= 0:
			if items[select][1] == 'watch':
				...
				control.execute('RunPlugin(plugin://plugin.video.zoro/?action=shows_traktHiddenManager)')
			elif items[select][1] == 'unfinishedEpisodeManager':
				control.execute('RunPlugin(plugin://plugin.video.zoro/?action=episodes_traktUnfinishedManager)')
			elif items[select][1] == 'unfinishedMovieManager':
				control.execute('RunPlugin(plugin://plugin.video.zoro/?action=movies_traktUnfinishedManager)')
