...
class AllDebrid:
	...
	def __init__(self):
		# Settings ids in this add-on use dotted keys (see resources/settings.xml).
		# Older builds mistakenly used legacy ids (alldebridtoken/alldebridusername),
		# which caused auth to "succeed" but not persist.
		self.token = getSetting('alldebrid.token')
		# Migrate legacy token if present
		if not self.token:
			legacy = getSetting('alldebridtoken')
			if legacy:
				self.token = legacy
				control.setSetting('alldebrid.token', legacy)
		self.timeout = 20
	...
	def auth(self, fromSettings=0):
		...
		# Build robust dialog text. Some progress dialogs/skins handle newlines and
		# color tags inconsistently (or hide text if the color setting is empty).
		pin = response.get('pin')
		check = response.get('check')
		if not pin or not check:
			# If API response is missing required fields, show a clear error instead
			# of an empty auth dialog.
			control.notification(message='AllDebrid: Failed to obtain PIN', icon=ad_icon)
			log_utils.log('AllDebrid: Failed to obtain PIN/check from API response', __name__, log_utils.LOGWARNING)
			if fromSettings == 1:
				control.openSettings('9.0', 'plugin.video.zoro')
			return
		pin_url = 'https://alldebrid.com/pin'
		# Validate highlight color (expects 8 hex digits). Fall back to plain text.
		hc = (self.highlight_color or '').strip()
		use_color = bool(re.match(r'^[0-9A-Fa-f]{8}$', hc))
		if use_color:
			line1 = getLS(32513) % (hc, pin_url)
			line2 = getLS(32514) % (hc, pin or '')
		else:
			line1 = '1) Open this link in a browser : %s' % pin_url
			line2 = '2) When prompted enter : %s' % (pin or '')
				# Optional QR like RD/PM (uses qrserver.com). If it fails, continue without QR.
		try:
			qr_base = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1&data='
			qr_icon = qr_base + quote_plus('https://alldebrid.com/pin?pin=%s' % (pin or ''))
			control.notification(message=pin_url, icon=qr_icon, time=15000)
		except:
			pass

# Keep original attributes used by auth_loop
		self.check = check
		self.pin = pin
		if control.setting('dialogs.usezorodialog') == 'true':
			# Use zoro progress window (no local QR image needed).
			self.progressDialog = control.getProgressWindow(getLS(40056), '', 1)
			self.progressDialog.set_controls()
		else:
			self.progressDialog = control.progressDialog
			self.progressDialog.create(getLS(40056))
		# Use separate lines to avoid newline rendering issues in some skins.
		try:
			self.progressDialog.update(-1, line1, line2)
		except:
			# Fallback for older progress dialog implementations
			self.progressDialog.update(-1, '%s\n%s' % (line1, line2))
		control.sleep(2000)
	...
	def user_transfers_to_listItem(self):
		sysaddon, syshandle = 'plugin://plugin.video.zoro/', int(argv[1])
