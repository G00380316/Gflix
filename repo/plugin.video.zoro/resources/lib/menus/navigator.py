...
class Navigator:
	...
	def root(self):
		...
		if getMenuEnabled('navi.changelog'): self.addDirectoryItem(32014, 'tools_ShowChangelog&name=zoro', 'userlists.png', 'DefaultAddonHelper.png', isFolder=False)
		self.endDirectory()
...
#		self.addDirectoryItem(f"{getLS(32002)} (IMDB)" if self.indexLabels else 32002, 'anime_TVshows&url=anime', 'tvshows.png', 'DefaultTVShows.png')
	...
	def traktSearchLists(self, media_type):
		...
		control.closeAll()
		if media_type == 'movies': control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=movies_PublicLists&url=%s,return)' % (quote_plus(url)))
		else: control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=tv_PublicLists&url=%s,return)' % (quote_plus(url)))
...
#		self.addDirectoryItem(32506, 'tools_contextzoroSettings', 'icon.png', 'DefaultAddonProgram.png', isFolder=False)
		#-- Providers - 5
	...
	def loggingNavigator(self):
		self.addDirectoryItem(32524, 'tools_viewLogFile&name=zoro', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32525, 'tools_clearLogFile', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
#		self.addDirectoryItem(32526, 'tools_ShowChangelog&name=zoro', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32527, 'tools_uploadLogFile&name=zoro', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
...
#		self.addDirectoryItem(32531, 'tools_ShowChangelog&name=FenomScrapers', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
	...
	def views(self):
		try:
			...
			title = getLS(32059)
			url = 'plugin://plugin.video.zoro/?action=tools_addView&content=%s' % content
	...
	def addDirectoryItem(self, name, query, poster, icon, context=None, queue=False, isAction=True, isFolder=True, isPlayable=False, isSearch=False, table=''):
		try:
			...
			url = 'plugin://plugin.video.zoro/?action=%s' % query if isAction else query
			...
			if queue: cm.append((queueMenu, 'RunPlugin(plugin://plugin.video.zoro/?action=playlist_QueueItem)'))
			if context: cm.append((getLS(context[0]), 'RunPlugin(plugin://plugin.video.zoro/?action=%s)' % context[1]))
			if isSearch: cm.append(('Clear Search Phrase', 'RunPlugin(plugin://plugin.video.zoro/?action=cache_clearSearchPhrase&source=%s&name=%s)' % (table, quote_plus(name))))
			cm.append(('[COLOR red]Zoro Settings[/COLOR]', 'RunPlugin(plugin://plugin.video.zoro/?action=tools_openSettings)'))
