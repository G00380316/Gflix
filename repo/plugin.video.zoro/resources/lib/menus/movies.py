...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
	...
	def unfinishedManager(self):
		try:
			...
			del window
			if selected_items:
				refresh = 'plugin.video.zoro' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=movies&url=%s,return)' % (quote_plus(url)))
...
#		url = self.persons_link + quote_plus(q)
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
...
####-Context Menu and Overlays-####
				...
				cm.append(('[COLOR red][B]zoro Settings[/B][/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
