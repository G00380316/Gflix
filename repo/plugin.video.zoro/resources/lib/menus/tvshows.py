...
#		if not self.tmdb_key: self.tmdb_key = ''
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=tvshows&url=%s,return)' % (quote_plus(url)))
...
#		url = self.persons_link + quote_plus(q)
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
