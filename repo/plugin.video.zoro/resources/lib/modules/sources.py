...
class Sources:
	...
	def play(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape=None):
		...
		try:
			control.sleep(200)
			if control.playlist.getposition() == 0 or control.playlist.size() == 1: playerWindow.clearProperty('zoro.preResolved_nextUrl')
			preResolved_nextUrl = playerWindow.getProperty('zoro.preResolved_nextUrl')
	...
	def sourceSelect(self, title, items, uncached_items, meta):
		...
		try:
			...
			if action == 'play_Item' and self.uncached_chosen != True:
				return self.playItem(title, items, chosen_source.getProperty('zoro.source_dict'), self.meta)
			else:
		...
		try:
			...
			for i in range(len(resolve_items)):
				try:
					...
					if homeWindow.getProperty('zoro.source_progress_is_alive') != 'true':
						try: self.progressDialog.close()
						except: pass
						del self.progressDialog
					from resources.lib.modules import player
				...
				try:
					...
					if url:
						...
						if player_hasVideo: # do not setPropery if user stops playback quickly because "onPlayBackStopped" is already called and won't be able to clear it.
							playerWindow.setProperty('zoro.preResolved_nextUrl', url)
...
#			control.notification(title=item_title, message='Removed %s duplicate sources from list' % (len(self.sources) - len(filter)))
	...
	def getConstants(self):
		self.metaProperty = 'plugin.video.zoro.container.meta'
		self.seasonProperty = 'plugin.video.zoro.container.season'
		self.episodeProperty = 'plugin.video.zoro.container.episode'
		self.titleProperty = 'plugin.video.zoro.container.title'
		self.imdbProperty = 'plugin.video.zoro.container.imdb'
		self.tmdbProperty = 'plugin.video.zoro.container.tmdb'
		self.tvdbProperty = 'plugin.video.zoro.container.tvdb'
		self.labelProperty = 'plugin.video.zoro.container.label'
