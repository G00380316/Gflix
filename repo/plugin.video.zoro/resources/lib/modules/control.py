...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('zoro_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('zoro_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getzoroVersion():
	return xbmcaddon.Addon('plugin.video.zoro').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.zoro').getAddonInfo('path'), 'resources', 'media', theme)

def genreIconPath():
	return joinPath(xbmcaddon.Addon('plugin.video.zoro').getAddonInfo('path'), 'resources', 'media', 'genre_media', 'icons')

def genrePosterPath():
	return joinPath(xbmcaddon.Addon('plugin.video.zoro').getAddonInfo('path'), 'resources', 'media', 'genre_media', 'posters')
...
def refresh_playAction(): # for zoro global CM play actions
	autoPlay = 'true' if setting('play.mode') == '1' else ''
	homeWindow.setProperty('zoro.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for zoro global CM library actions
	homeWindow.setProperty('zoro.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('zoro.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear zoro.log if setting to reverse has been changed
	if homeWindow.getProperty('zoro.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('zoro.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://plugin.video.zoro/?action=tools_clearLogFile)')
