...
plugin = 'plugin://plugin.video.zoro/'
...
class CheckSettingsFile:
	def run(self):
		try:
			control.log('[ plugin.video.zoro ]  CheckSettingsFile Service Starting...', LOGINFO)
			window.clearProperty('zoro_settings')
			...
			return control.log('[ plugin.video.zoro ]  CheckSettingsFile Service Finished', LOGINFO)
		except:
			log_utils.error()
...
class SettingsMonitor(control.monitor_class):
	def __init__ (self):
		...
		window.setProperty('zoro.debug.reversed', control.setting('debug.reversed'))
		control.log('[ plugin.video.zoro ]  Settings Monitor Service Starting...', LOGINFO)
...
class ReuseLanguageInvokerCheck:
	def run(self):
		control.log('[ plugin.video.zoro ]  ReuseLanguageInvokerCheck Service Starting...', LOGINFO)
		try:
			import xml.etree.ElementTree as ET
			from resources.lib.modules.language_invoker import gen_file_hash
			addon_xml = control.joinPath(control.addonPath('plugin.video.zoro'), 'addon.xml')
			tree = ET.parse(addon_xml)
			root = tree.getroot()
			current_addon_setting = control.addon('plugin.video.zoro').getSetting('reuse.languageinvoker')
			try: current_xml_setting = [str(i.text) for i in root.iter('reuselanguageinvoker')][0]
			except: return control.log('[ plugin.video.zoro ]  ReuseLanguageInvokerCheck failed to get settings.xml value', LOGINFO)
			...
			if current_xml_setting == current_addon_setting:
				return control.log('[ plugin.video.zoro ]  ReuseLanguageInvokerCheck Service Finished', LOGINFO)
			...
			for item in root.iter('reuselanguageinvoker'):
				...
				control.log('[ plugin.video.zoro ]  ReuseLanguageInvokerCheck Service Finished', LOGINFO)
...
class AddonCheckUpdate:
	def run(self):
		control.log('[ plugin.video.zoro ]  Addon checking available updates', LOGINFO)
		try:
			import re
			import requests
			repo_xml = requests.get('')
			if not repo_xml.status_code == 200:
				return control.log('[ plugin.video.zoro ]  Could not connect to remote repo XML: status code = %s' % repo_xml.status_code, LOGINFO)
			repo_version = re.findall(r'<addon id=\"plugin.video.zoro\".+version=\"(\d*.\d*.\d*)\"', repo_xml.text)[0]
			local_version = control.getzoroVersion()[:5] # 5 char max so pre-releases do try to compare more chars than github version
			...
			if check_version_numbers(local_version, repo_version):
				while control.condVisibility('Library.IsScanningVideo'):
					control.sleep(10000)
				control.log('[ plugin.video.zoro ]  A newer version is available. Installed Version: v%s, Repo Version: v%s' % (local_version, repo_version), LOGINFO)
				control.notification(message=control.lang(35523) % repo_version)
			return control.log('[ plugin.video.zoro ]  Addon update check complete', LOGINFO)
		except:
			log_utils.error()
...
class VersionIsUpdateCheck:
	def run(self):
		try:
			...
			if isUpdate:
				CheckPackages().run()
				window.setProperty('zoro.updated', 'true')
				curVersion = control.getzoroVersion()
				...
				if do_cacheClear:
					...
					if cleared:
						...
						control.log('[ plugin.video.zoro ]  Forced traktsync clear for version update complete.', LOGINFO)
					if clr_fanarttv:
						...
						control.log('[ plugin.video.zoro ]  Forced fanarttv.db clear for version update complete.', LOGINFO)
				control.setSetting('trakt.message2', '') # force a settings write for any added settings that may have been added in new version
				control.log('[ plugin.video.zoro ]  Forced new User Data settings.xml saved', LOGINFO)
				control.log('[ plugin.video.zoro ]  Plugin updated to v%s' % curVersion, LOGINFO)
		except:
			log_utils.error()

class SyncTraktCollection:
	def run(self):
		control.log('[ plugin.video.zoro ]  Trakt Collection Sync Starting...', LOGINFO)
		...
		control.log('[ plugin.video.zoro ]  Trakt Collection Sync Complete', LOGINFO)

class LibraryService:
	def run(self):
		control.log('[ plugin.video.zoro ]  Library Update Service Starting (Update check every 6hrs)...', LOGINFO)
...
class SyncTraktService:
	def run(self):
		...
		control.log('[ plugin.video.zoro ]  Trakt Sync Service Starting (sync check every %s minutes)...' % service_syncInterval, LOGINFO)
...
class CheckUndesirablesDatabase:
	def run(self):
		...
		try:
			control.log('[ plugin.video.zoro ]  CheckUndesirablesDatabase Service Starting', LOGINFO)
			old_database = Undesirables().check_database()
			if old_database: add_new_default_keywords()
			control.log('[ plugin.video.zoro ]  CheckUndesirablesDatabase Service Finished', LOGINFO)
		except:
			log_utils.error()

class PremAccntNotification:
	def run(self):
		control.log('[ plugin.video.zoro ]  Debrid Account Expiry Notification Service Starting', LOGINFO)
		...
		return control.log('[ plugin.video.zoro ]  Debrid Account Expiry Notification Service Finished', LOGINFO)
...
class CheckPackages:
	def run(self):
		...
		for item in files:
			if '.zoro' in item and item.endswith('zip'):
...
def main():
	while not control.monitor.abortRequested():
		control.log('[ plugin.video.zoro ]  Service Started', LOGINFO)
		schedTrakt = None
		libraryService = None
...
#			AddonCheckUpdate().run()
	...
	control.log('[ plugin.video.zoro ]  Settings Monitor Service Stopping...', LOGINFO)
	del syncTraktService # prob does not kill a running thread
	control.log('[ plugin.video.zoro ]  Trakt Sync Service Stopping...', LOGINFO)
	if libraryService:
		del libraryService # prob does not kill a running thread
		control.log('[ plugin.video.zoro ]  Library Update Service Stopping...', LOGINFO)
	if schedTrakt:
		schedTrakt.cancel()
	control.log('[ plugin.video.zoro ]  Service Stopped', LOGINFO)

try:
	kodiVersion = control.getKodiVersion(full=True)
	addonVersion = control.addon('plugin.video.zoro').getAddonInfo('version')
#	repoVersion = control.addon('repository.zoro').getAddonInfo('version')
#	fsVersion = control.addon('script.module.fenomscrapers').getAddonInfo('version')
	log_utils.log('########   CURRENT zoro VERSIONS REPORT   ########', level=LOGINFO)
#	log_utils.log('##   Platform: %s' % str(sys_platform), level=LOGINFO)
	...
	log_utils.log('##   plugin.video.zoro Version: %s' % str(addonVersion), level=LOGINFO)
#	log_utils.log('##   repository.zoro Version: %s' % str(repoVersion), level=LOGINFO)
#	log_utils.log('##   script.module.fenomscrapers Version: %s' % str(fsVersion), level=LOGINFO)
	log_utils.log('######   zoro SERVICE ENTERING KEEP ALIVE   #####', level=LOGINFO)
except:
	log_utils.log('## ERROR GETTING zoro VERSION - Missing Repo or failed Install ', level=LOGINFO)
