...
#		if not self.tmdb_key: self.tmdb_key = ''
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zoro/?action=collections&url=%s,return)' % (quote_plus(url)))
...
####-Context Menu and Overlays-####
				...
				cm.append(('[COLOR red]zoro Settings[/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
####################################
				...
				if is_widget: item.setProperty('iszoro_widget', 'true')
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False):
		try:
			...
			url = 'plugin://plugin.video.zoro/?action=%s' % action
			cm = []
